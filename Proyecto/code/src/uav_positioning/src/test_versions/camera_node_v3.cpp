...
void CameraNode::callback(const fiducial_msgs::FiducialTransformArray& msg) 
{
    ...
    for (auto &marker:msg.transforms) {
        ...
        //cout << "RESULTADO ORIENTACION MARKER: " <<  marker.fiducial_id << ": X-> " << q_result.getX() << ": Y-> " << q_result.getY()<< ": Z-> " << q_result.getZ()<< ": W-> " << q_result.getW() << endl;

        cout << "RESULTADO ORIENTACION MARKER: " <<  marker.fiducial_id << ", RADS:" << q_result.getAngle() << endl;
