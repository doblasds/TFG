...
void CameraNode::callback(const fiducial_msgs::FiducialTransformArray& msg) 
{
    ...
    for (auto &marker:msg.transforms) {
        ...
        bool started = 
        pose_cache.orientation.x != 0.0 || \
        pose_cache.orientation.y != 0.0 || \
        pose_cache.orientation.z != 0.0 || \
        pose_cache.orientation.w != 0.0;
        //CALCULO DEL ERROR DEL MARCADOR
        float threshold = 0.8; 
        bool error = 
        abs( pose_cache.orientation.x - q_result.getX() ) > threshold || \
        abs( pose_cache.orientation.y - q_result.getY() ) > threshold || \
        abs( pose_cache.orientation.z - q_result.getZ() ) > threshold || \
        abs( pose_cache.orientation.w - q_result.getW() ) > threshold;
